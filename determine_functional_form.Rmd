...
Because intercept terms are so common in linear models, the formulas above include one by default. If you don't want an intercept term, add a `0` to your formula.
...
Note that the constant for Fair (the worst quality) diamonds is 0. By convention, the value that would correspond to the lowest level of a factor is added to `a_0` and the values for the other levels are relative to the value for the lowest.

In the next section, we'll visualize `clarity` and `color` and discuss visualizing discrete predictors in more detail. 

### Visual checks

Now that you understand the form of `f_cut()`, we can be more precise about what to look for when visualizing a potential discrete predictor. Here's a brief checklist:

* The smooth lines of continuous predictors for the levels of the discrete predictor should all be approximately linear.

  Instead of one smooth line without the discrete predictor, you would now have a smooth line for each level. Each of these smooth lines should be linear for the relationship to be approximated well by a linear function.
