...
Earlier, we said that the first step in model-building is to determine which function family would describe your data well. 
...
Earlier, we introduced the idea of residuals and RMSE, but didn't really justify using RMSE to find our model. One downside of models that minimize RMSE is that they're very sensitive to outliers. 

Here, we've added an outlier our data.
...
df_outlier %>%
  ...
  df_scale_coord +
  labs(title = "Original data, plus an outlier")
```

Adding a single point changes our model significantly.
...
* Find the functional form of your data. Visualize your data to determine which function family would approximate your data well.
* Choose a metric. 
* Use an R function to find find the specific function in your chosen function family that minimizes your chosen metric. 
