...
When building a supervised model, your job as the modeler is to find a function that approximates your data. Functions map from one or more variables (e.g., time) to another (e.g., amount of traffic). The approximation function can be used to understand and describe your data. It can also be used to make predictions of the response variable for a given set of predictor variables.

There are an infinite number of types of functions, so how do you know where to look? The first step is to explore your data, understand its functional form, and decide on a _function family_. Function families are sets of functions with the same functional form. In [link to EDA on functional form], we'll talk more about function families and how to use EDA to choose a function family with which to approximate your data.
...
From the many residuals, we need a single error metric. One common option is to find the _root mean-square error (RMSE)_. To calculate the RMSE, square each residual, find the mean, and then take the square root of that mean:
...
The MAE model isn't as disproportionately affected by our outlier as the RMSE model. `lm()` does not have the option of using the MAE error metric, but the R function `MASS::rlm()` (where the "r" is for "robust") offers are range of different error metrics that are less sensitive to outliers than RSME.
