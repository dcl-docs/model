...
a_1_rmse <- coef(model_lm)[["x"]]
```

Here's the model that `lm()` came up with:

`y = `r a_0_rmse` + `r a_1_rmse` * x`
...
Again, `a_0` is the intercept, so we know that our model predicts that `y` will be `r a_0_rmse` when `x = 0`. `a_1` is the slope, which means that our model predicts a `r a_1_rmse` increase in `y` each time `x` increases by 1.   
