...
The `diamonds` dataset, in the ggplot2 package, contains information on `r nrow(diamonds)` round diamonds from the [Loose Diamonds Search Engine](https://www.diamondse.info). 
...
Each row of `diamonds` represents a single diamond, and contains data on that diamond's price and features, including a set of features called the four C's: `carat`, `cut`, `clarity`, and `color`. 

Over the next several chapters, we'll try to understand and model the relationship between the four C's and diamond price. Before diving into modeling, however, it's critical to understand your data. In this chapter, we'll conduct some exploratory data analysis to understadn our variables, look for errors adn outliers, and restrict our data to where most of the diamonds are. 

## Understand your variables

### Read the documentation

When working with a new dataset, the first step is to read the documentation. Because `diamonds` is built into ggplot2, the diamonds help page (`?diamonds`) contains the official documentation. The [Loose Diamonds Search Engine](https://www.diamondse.info), the original source, also contains additional helpful information about the variables.  
...
It's also useful to understand that there are many [different diamonds shapes](https://www.diamondse.info/), but `diamonds` contains only round diamonds. `diamonds` also only tracks loose diamonds not set in any type of jewelry. 
...
Notice that there are no `NA`'s in the data (`summary()` shows the number of `NA`'s in each column if there are any). Also take a look at the minimum and maximum values for each variable. `carat` and `price` both cover a large range of values. `x`, `y`, and `z` all have 0 as a minimum value, even though it's impossible for a diamond to actually have a 0mm length, width, or depth. `y` and `z` also have very large maximums. We'll investigate these implausible or impossible values in the next section. 
...
There are definite outliers for `x`, but they aren't too extreme. A diamond with an 10mm (.39 in) length seems plausible.
...
To understand the distribution of continuous varibles, we use histograms. First, let's look at `carat`.
...
When you build a model, it's a good idea to focus on the data you care the most about. There's a big difference between a 0.2 carat diamond and a 5 carat diamond. We care about modeling the relationship between price and the 4 C's for most of our diamonds, not for the outliers. Let's limit our data so that we only account for 99% of the diamonds, ignoring the 1% that account for over half of the range.
...
Notice how spiky the distribution is. By labeling the `x` axis, we can get more clarity about where these spikes are.
...
The spikes appear at or slightly above the even sizes of 0.3, 0.4, 0.5, 0.7, 0.9, 1.0, 1.2, 1.5, 1.7, and 2.0 carats. The diamonds in this dataset are cut from raw diamonds, so this could be becuase diamonds tend to be cut at these sizes. Maybe people tend to buy diamonds of these sizes, so the diamond manufacturers take this into account. 
...
Again, we see a distribution with a long tail, even though we've filtered our data to the 99% of diamonds. Interestingly, there's a gap in the distribution around $1500. There are no diamonds priced between $1455 and 1545, probably caused by an error importing the data. 
