---
nocite: |
  @gelman-2021, @kuhn-2019, @kuhn-tmr
---
...
formulas <- 
  ...
  mutate(
    n_coefs = 
      map_int(
        formula,
        ~ model.matrix(as.formula(.), data = df %>% slice(1)) %>% ncol()
      )
  )
...
bp_errors <- 
  tibble(
    formula = str_glue("bpxosy ~ poly(ridageyr, degree = {1:6})"),
    n_coefs = 
      map_int(
        formula,
        ~ model.matrix(as.formula(.), data = bp_model) %>% ncol()
      ),
    mae =
      map(formula, ~ lmrob(as.formula(.), data = bp_model)) %>% 
      map_dbl(~ mean(abs(bp_new$bpxosy - predict(., newdata = bp_new))))
  )
```
...
* __k-Fold__: In this method, the original data is randomly partitioned into k disjoint sets of roughly equal size, called folds. The modeling is then done k times with each fold serving as test set and the remaining k - 1 folds serving as the training set. Typical values for k are 5 or 10, corresponding to 80% or 90% of the original data being used for the training set.

With Monte Carlo and bootstrap cross-validation, the train and test process is repeated multiple times with different random partitions. With k-fold cross-validation, the whole process of k repetitions can itself be repeated. In each of the three methods, the results of the repetitions are combined into an average estimate of predictive performance and a standard error.

For our diamonds data, we'll use k-fold cross-validation with k = 10 folds and just one repeat. The `vfold_cv()` function in the tidymodels [rsample](https://rsample.tidymodels.org/) package will do this for us (it uses "v" instead of "k").
...
v <- model_metric(formula = log(price) ~ log(carat))
...
rmse_lm <- 
  formulas %>% 
  mutate(metric = map(formula, ~ model_metric(formula = as.formula(.)))) %>% 
  unnest(cols = metric) %>% 
...
elpd_stan_glm <- 
  formulas %>% 
  mutate(
    elpd = map_dbl(loos, ~ .$estimates["elpd_loo", "Estimate"]),
    elpd_se = map_dbl(loos, ~ .$estimates["elpd_loo", "SE"])
  )
...
As we've seen, in supervised learning were the goal is to make accurate predictions, it is important to understand your data, properly prepare your data, and then explore possible function families with which to model it. A good resource to learn more about this process is [Feature Engineering and Selection](https://bookdown.org/max/FES/).

We used some functions from the [tidymodels](https://www.tidymodels.org/) ecosystem of R packages for modeling. A good source to go deeper is [Tidy Modeling with R](https://www.tmwr.org/).

A good introduction to Bayesian modeling and data analysis is [Regression and Other Stories](https://avehtari.github.io/ROS-Examples/index.html). Two of the authors are active in the Stan project, and the book uses `stan_glm()` for both linear and other general linear models. Tidyverse implementations of examples from the book are available [here](https://github.com/behrman/ros#readme).
